...
static constexpr uint32_t kMinProbeRowEachCrossProductBatch = 8;
} // namespace
...
bool NestedLoopJoinProbe::getBuildData(ContinueFuture* future) {
  ...
  if (isSingleBuildRow()) {
    crossProductMode_ = CrossProductMode::kSingleBuildRow;
  } else if (
      isSingleBuildVector() &&
      (kMinProbeRowEachCrossProductBatch * (*buildVectors_)[0]->size() <
       outputBatchSize_)) {
    crossProductMode_ = CrossProductMode::kSingleBuildBatch;
  } else {
    crossProductMode_ = CrossProductMode::kMultiBuildBatch;
  }
...
bool NestedLoopJoinProbe::addToOutput() {
  ...
  while (!hasProbedAllBuildData()) {
    ...
    for (auto i = filterResultRow_; i < decodedFilterResult_.size(); ++i) {
...
RowVectorPtr NestedLoopJoinProbe::getNextCrossProductBatch(
  ...
  if (crossProductMode_ == CrossProductMode::kSingleBuildRow) {
    return genCrossProductSingleBuildRow(
        buildVector, outputType, probeProjections, buildProjections);
  }
  if (crossProductMode_ == CrossProductMode::kSingleBuildBatch) {
